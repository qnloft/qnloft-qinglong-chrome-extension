...
class StorageManager {
    ...
    async getConfig() {
        ...
        // 如果clientSecret存在且看起来已加密（包含Base64字符），则解密
        if (config.clientSecret && config.clientSecret.length > 50) {
            try {
                config.clientSecret = await cryptoUtils.decrypt(config.clientSecret);
            } catch (error) {
                console.error('解密clientSecret失败:', error);
    ...
    async saveConfig(config) {
        ...
        if (configToSave.clientSecret && configToSave.clientSecret.trim()) {
            try {
                // 如果看起来还没加密（长度较短），则加密
                if (configToSave.clientSecret.length <= 50) {
