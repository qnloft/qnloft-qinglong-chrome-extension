...
- 🔄 **自动同步** - 定时自动检测Cookie变化并同步到青龙面板，支持批量和手动触发
- 🌐 **多网站支持** - 灵活配置多个网站，独立管理每个站点的同步设置和环境变量
- 📋 **环境变量管理** - 完整的青龙面板环境变量CRUD操作，支持搜索、过滤、批量操作
- 🧭 **更佳表单体验** - 目标URL支持浏览器历史记录联想；环境变量名从青龙面板下拉选择
- 🔒 **安全可靠** - 采用AES-256-GCM加密存储敏感信息，内置错误处理和自动重试
- 📊 **日志与统计** - 记录详细的同步历史和成功率，实时追踪每次操作结果
...
- 一键同步/删除
- 目标URL支持浏览器历史记录联想
- 环境变量名称支持从青龙面板下拉选择（展示“名称 — 值前缀”以区分重名）
...
- 环境变量名称：从下拉中选择青龙面板的变量名（如：`JD_COOKIE`，从青龙自动加载）
- 勾选"启用此网站"和"启用自动同步"
...
- **网站名称：** 便于识别的名称
- **目标URL：** 获取Cookie的网站地址
- **环境变量名称：** 从下拉中选择在青龙面板中对应的变量名（自动加载，支持同名，通过值前缀区分）
- **启用此网站：** 是否启用该网站配置
- **自动同步：** 是否参与自动同步

**注意事项：**
- 环境变量需要先在青龙面板中创建；扩展会自动拉取并展示为下拉列表
- URL应该是登录后的页面地址
- 确保已在目标网站登录
